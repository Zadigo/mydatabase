      ...
      <v-dialog id="settings-modal" v-model="showSheetSettings" width="300">
        <v-card>
          <v-card-text>
            <div class="row">
              <div class="col-12">
                <h2 v-if="isCreation" class="h4 mb-3">
                  New spreadsheet
                </h2>
                <h2 v-else class="h4 mb-3">
                  Change spreadsheet
                </h2>
          ...
          <v-card-actions class="justify-content-end">
            <v-btn @click="showSheetSettings = false">
              Cancel
            </v-btn>
        ...
        <v-card>
          <v-card-text>
            <div class="row">
              ...
              <div class="col-12">
                <v-text-field v-model="requestData.name" class="mb-1" variant="outlined" label="Your connection's name" hide-details />

                <v-switch v-model="uploadChoice" label="Upload using a csv file" inset hide-details />
                <v-file-input v-if="uploadChoice" v-model="requestData.csv_file" label="Choose a csv file" variant="solo" accept=".csv" />

                <div v-else>
                  <v-text-field v-model="requestData.endpoint_url" class="mb-2" type="url" variant="outlined" label="Add an API endpoint url" hide-details />
                  <v-text-field v-model="requestData.endpoint_data_key" variant="outlined" label="Choose the data entrypoint key" hide-details />
                </div>
          ...
          <v-card-actions class="justify-content-end">
            <v-btn @click="showCsvSettings = false">
              Cancel
            </v-btn>
            <v-btn variant="tonal" @click="handleCreateConnection">
              Validate
            </v-btn>
          </v-card-actions>
        </v-card>
