        ...
        <thead>
          <tr>
            <th v-for="column in blockDetails.visible_columns" :key="column" class="text-left">
              <v-btn variant="tonal" rounded @click="handleShowColumnActionModal(column)">
                {{ column }}
              </v-btn>
            </th>
          </tr>
    ...
    <teleport to="body">
      <v-dialog id="search-records-modal" v-model="showSearchRecordsModal" width="400">
        <v-card title="Select columns">
          ...
          <v-card-actions>
            <v-btn @click="showSearchRecordsModal = false">
              Cancel
            </v-btn>
    ...
    <teleport to="body">
      <v-dialog id="column-update-modal" v-model="showColumnActionsModal" width="500">
        <v-card>
          ...
          <v-card-actions>
            <v-btn @click="showColumnActionsModal = false">
              Cancel
            </v-btn>
            <v-btn variant="tonal" @click="handleUpdateColumn">
              Validate
            </v-btn>
          </v-card-actions>
        </v-card>
