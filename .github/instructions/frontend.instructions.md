---
applyTo: "frontend/**/*"
description: "Use when editing the Nuxt frontend, including pages, components, stores, composables, tests, and app configuration under frontend/."
---
...
- The frontend is a Nuxt 4 app configured in [frontend/nuxt.config.ts](frontend/nuxt.config.ts). Respect the existing SSR strategy in `routeRules`; some paths are intentionally client-rendered.
...
- Available scripts are defined in [frontend/package.json](frontend/package.json). Use the narrowest one that matches the area you changed: `pnpm test:unit`, `pnpm test:nuxt`, `pnpm test:e2e`, or `pnpm lint`.
...
- When changing a route or rendering behavior, review [frontend/nuxt.config.ts](frontend/nuxt.config.ts) first so you do not accidentally break prerendered or SSR-disabled pages.
