<template>
  <div id="upload">
    <div class="space-y-2">
      ...
      <!-- Name -->
      <nuxt-input v-model="editedDocument.name" variant="subtle" class="w-full" placeholder="Document Name" />
      ...
      <nuxt-tabs :items="items">
        <template #content="{ item }">
          ...
          <div v-if="item.label === 'File'" id="fileupload" class="py-3 space-y-2">
            ...
            <nuxt-input v-model="editedDocument.entry_key" variant="subtle" class="w-full" placeholder="Entry Key e.g. results, data" />
            
            {{ processedData }}

            <!-- CSV/Json -->
            <nuxt-file-upload v-model="editedDocument.file" label="CSV, JSON" description="Select a CSV or JSON file to upload" class="w-full min-h-48" accept=".csv,.json,.xlsx" />

            <nuxt-button :loading="isProcessing" @click="process(editedDocument.file)">Test process</nuxt-button>
          </div>
          ...
          <div v-else-if="item.label === 'Url'" id="url" class="py-3">
            ...
            <nuxt-input v-model="editedDocument.url" :loading="true" class="w-full" placeholder="Url hosting the CSV or JSON file" />
          ...
          <div v-else-if="item.label === 'Google sheet'" id="googlesheet" class="space-y-2 py-3">
            <nuxt-input v-model="editedDocument.google_sheet_id" class="w-full" placeholder="Google Sheet ID" />
          </div>
        </template>
      </nuxt-tabs>
    </div>
...
const editedDocument = defineModel<NewDocument>('editedDocument', { required: true })
...
watch(() => editedDocument.value, (newDoc) => {
  console.log(newDoc)
  if (isDefined(wsObject) && newDoc.url !== '') {
    ...
    if(isDefined(wsObject.data)) {
      ...
      if (data?.action === 'checkedout_url') {
        editedDocument.value.using_columns = data.columns.type_options
